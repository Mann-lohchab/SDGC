...
for i in range(N):
    for j in range(i+1, N):
        ...
        if d < 15:  # hard constraint
            adj[u].append(v)
            adj[v].append(u)
        elif d < 30:  # soft constraint
            dist2[u].append(v)
            dist2[v].append(u)
...
def conflict_penalty(v, c):
    ...
    for u in dist2[v]:
        if colors[u] is not None and abs(colors[u] - c) < 2:
            penalty += 3
...
print("Drone Colors used:", len(set(colors.values())))
...
plt.title(f"Drone SDGC (Colors={len(set(cs))}, Violations={violations()})")
plt.colorbar(sc)
...
plt.savefig("drone_sdgc.png", dpi=300, bbox_inches='tight')
plt.show()
